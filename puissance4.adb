...
package body Puissance4 is
   ...
   begin
      if C.Qui=Joueur1 then E.G(C.Coordonnees(1),C.Coordonnees(2)):=1;
      elsif C.Qui=Joueur2 then E.G(C.Coordonnees(1),C.Coordonnees(2)):=2;
         --else raise Erreur;
      end if;
      return E;
   end Jouer;
   ...
   begin
      Put("joue dans la " & Integer'Image(C.Coordonnees(2)));New_Line;
   end Affiche_Coup;
